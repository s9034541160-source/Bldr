...
tools_adapter: Optional[ToolsAdapter] = None
...
async def execute_tool_job(job_id: str, coordinator: CoordinatorAgent, 
...
async def list_tools():
    ...
    try:
        ...
        if hasattr(unified_tools, 'list_available_tools'):
            info = unified_tools.list_available_tools()
...
async def cleanup_completed_jobs():
    ...
    try:
        ...
        cleaned = 0
        to_remove = []
        for job_id in list(r.smembers(REDIS_COMPLETED_SET)):
            raw = r.get(_job_key(job_id))
            if not raw:
