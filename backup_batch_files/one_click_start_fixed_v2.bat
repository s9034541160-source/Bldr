...
REM Skip Neo4j auto-start and setup as requested
echo [INFO] Skipping Neo4j auto-start and setup. Please start it manually if needed.
...
echo   - Neo4j: http://localhost:7474 (if started manually)
echo   - Qdrant: http://localhost:6333 (if Docker is working)
echo   - FastAPI Backend: http://localhost:8000
echo   - Frontend Dashboard: http://localhost:3001
echo.
echo Credentials:
echo   - Neo4j: neo4j/neopassword (if started manually)
echo.
echo To start Neo4j manually:
echo   1. Open Neo4j Desktop
echo   2. Create or start a database instance
echo   3. Make sure it's listening on port 7687
echo   4. Run 'python setup_neo4j.py' to configure the password
echo.
