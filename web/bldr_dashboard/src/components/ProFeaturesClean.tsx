...
const ProFeatures: React.FC = () => {
    ...
    if (projectId) {
      ...
      try {
        ...
        if (smetaFiles.length > 0) {
          ...
          // Pre-fill budget form if we're on the budget tab
          if (activeTab === 'budget') {
            const project = projects.find(p => p.id === projectId);
            if (project) {
            ...
            // Check if we have parsed estimate results to pre-fill positions
            const estimateResult = projectResults.find(r => r.type === 'estimate_parse');
            if (estimateResult && estimateResult.data && estimateResult.data.merged_positions) {
              // Convert merged positions to form format
              const positions = estimateResult.data.merged_positions.map((pos: any) => ({
                code: pos.code || '',
                name: pos.name || '',
                unit: pos.unit || '',
                quantity: pos.quantity || 0,
                unit_cost: pos.unit_cost || 0
              }));
              
              // Set positions in the form
              budgetForm.setFieldsValue({
                positions: positions
              });
              
              message.success('Позиции сметы автоматически загружены из результатов проекта');
            }
          }
          
          // Pre-fill PPR form if we're on the PPR tab
          if (activeTab === 'ppr') {
            const project = projects.find(p => p.id === projectId);
            if (project) {
              pprForm.setFieldsValue({
                project_name: project.name,
                project_code: project.code,
                location: project.location || 'Екатеринбург'
              });
            }
          }
          
          // Pre-fill tender analysis form if we're on the tender tab
          if (activeTab === 'tender_analysis_2') {
            const project = projects.find(p => p.id === projectId);
            if (project) {
              tenderForm.setFieldsValue({
                project_id: project.id,
                tender_name: project.name,
                // We could pre-fill other fields based on project data
              });
            }
          }
        }
  ...
  // Add a section to display project results
  const renderProjectResults = () => {
    if (!selectedProject || projectResults.length === 0) return null;
    
    return (
      <Card title="Результаты проекта" size="small" style={{ marginTop: 16 }}>
        {isResultsLoading ? (
            ...
            columns={[
              {
                ...
                render: (type: string) => {
                  switch (type) {
                    case 'estimate_parse': return 'Парсинг смет';
                    case 'budget_calculation': return 'Расчет бюджета';
                    case 'ppr_generation': return 'Генерация ППР';
                    default: return type;
                  }
                }
              },
              ...
              },
              {
                ...
                render: (_, record: any) => (
                  <Button 
                    size="small" 
                    onClick={() => {
                      // In a real implementation, we would show details or allow download
                      message.info(`Результат типа "${record.type}" создан ${new Date(record.created_at).toLocaleString()}`);
                    }}
                  >
                    Просмотр
                  </Button>
  ...
  // Handle batch estimate parsing with result saving
      ...
      if (response.status === 'success') {
        ...
        if (selectedProject) {
          try {
            ...
            await apiService.saveProjectResult(selectedProject, 'estimate_parse', response);
            message.success('Результаты сохранены в проект');
            
            // Refresh project results
            const results = await apiService.getProjectResults(selectedProject);
            setProjectResults(results);
          } catch (saveError) {
  ...
  // Handle budget calculation with result saving
    ...
    try {
      ...
      if (response.status === 'success') {
        ...
        if (selectedProject) {
          try {
            // Save results to project
            await apiService.saveProjectResult(selectedProject, 'budget_calculation', response);
            message.success('Результаты расчета бюджета сохранены в проект');
            ...
            console.error('Error saving results to project:', saveError);
            message.warning('Не удалось сохранить результаты расчета бюджета в проект');
          }
        }
      } else {
  ...
  // Handle PPR generation with result saving
    ...
    try {
      ...
      if (response.status === 'success') {
        ...
        if (selectedProject) {
          try {
            // Save results to project
            await apiService.saveProjectResult(selectedProject, 'ppr_generation', response);
            message.success('Результаты генерации ППР сохранены в проект');
            ...
            console.error('Error saving results to project:', saveError);
            message.warning('Не удалось сохранить результаты генерации ППР в проект');
          }
        }
      } else {
